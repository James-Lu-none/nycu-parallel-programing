             ...
             cl_program *program)
{
    ...
    // since host_fe may be called multiple times with same filter and input, so we can cache them if they are the same
    if (cached_filter == NULL || filter != cached_filter || filter_width != cached_filter_width)
    {
        ...
        while (new_filter_width > 1)
        {
            ...
            for (int i = 0; i < w; ++i)
            {
                if (work_filter[i] != 0.0f
                    || work_filter[last * w + i] != 0.0f
                    || work_filter[i * w] != 0.0f
                    || work_filter[i * w + last] != 0.0f)
                {
                    all_zeros = 0;
                    break;
                }
            ...
            {
                memcpy(&work_filter[i * new_filter_width], &work_filter[(i + 1) * old_width + 1], new_filter_width * sizeof(float));
            }
        }

        size_t filter_bytes = (size_t)(new_filter_width * new_filter_width) * sizeof(float);
        cl_int status;
        cached_d_filter = clCreateBuffer(*context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, filter_bytes, work_filter, &status);
    ...
    if (cached_input == NULL || input_image != cached_input || image_height != cached_h || image_width != cached_w)
    {
        if (cached_d_input){
        ...
        cl_int status;
        cached_d_input = clCreateBuffer(*context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, image_bytes, input_image, &status);
    ...
    const char *kernel_name = (new_filter_width == 3)?"convolution_3x3":"convolution";
